...
for i in range(config["iterations"]):
    ...
    original_expr = model_output["Alpha Expression"]
    reversed_expr = alpha_utils.reverse_fastexpr(original_expr)

    alpha_original = copy.deepcopy(alpha)
    alpha_original["regular"] = original_expr

    alpha_reversed = copy.deepcopy(alpha)
    alpha_reversed["regular"] = reversed_expr

    simulation_results = ace.simulate_alpha_list(
        brain_session,
        [alpha_original, alpha_reversed],
        limit_of_concurrent_simulations=2,
    )

    res_orig = simulation_results[0]
    res_rev = simulation_results[1]

    sharpe_orig = float(res_orig["is_stats"]["sharpe"].iloc[0])
    sharpe_neg = float(res_rev["is_stats"]["sharpe"].iloc[0])

    if sharpe_neg > sharpe_orig:
        console.print(
            ">> Negated Hypothesis performed better. Switching context.", style="yellow"
        )
        simulation_result = res_rev
        model_output["Alpha Expression"] = reversed_expr

        flag_msg = " [AUTO-CORRECT: Empirical simulation proved the reverse direction was more profitable.]"
        
        model_output["Iteration Logic"] += flag_msg
        model_output["Economic Rationale"] += flag_msg

    else:
        console.print(
            ">> Original Hypothesis performed better. Keeping original.", style="yellow"
        )
        simulation_result = res_orig
        model_output["Alpha Expression"] = original_expr
